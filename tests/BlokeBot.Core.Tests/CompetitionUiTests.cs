...
public sealed class CompetitionUiTests
{
    ...
    [Test]
    public async Task CreateDraftCompetition_ScrollsWithTheComposerOutsideEverySaveRegion()
    {
        await using var database = await SqliteBlokeBotDbFactory.CreateAsync();
        var hostId = await SeedCompetitionHostAsync(database);
        using var context = UiTestContextFactory.Create(database, hostId);
        ...
        var create = page.Find("[data-action='create-competition']");
        create.TextContent.Trim().ShouldBe("Create draft competition");
        create.Closest("[data-save-scope]").ShouldBeNull();
        create.Closest(".sticky-save-region").ShouldBeNull();
        page.FindAll("[data-save-scope]").ShouldBeEmpty();
        create.ParentElement!.ClassList.ShouldContain("competition-composer__submit");
    }
