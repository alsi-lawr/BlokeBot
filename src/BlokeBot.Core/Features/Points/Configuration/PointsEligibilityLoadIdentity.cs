...
{
    private PointsEligibilityLoadIdentity(string hostLogin) => HostLogin = hostLogin;

    public string HostLogin { get; }

    public static PointsEligibilityLoadIdentity? From(string hostLogin) =>
        string.IsNullOrWhiteSpace(hostLogin) ? null : new(hostLogin.Trim().ToLowerInvariant());
}
