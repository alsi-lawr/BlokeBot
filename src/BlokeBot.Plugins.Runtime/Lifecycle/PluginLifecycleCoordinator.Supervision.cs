    ...
    )
    {
        try
        {
            ...
            )
            {
                ...
                var previous = _snapshots.StopAdmission(next);
                ...
                {
                    _ = await StopRuntimeAsync(next, previous, CancellationToken.None);
                    _ = PublishConflict(current.PluginId, conflict.Current);
                    return;
                }
