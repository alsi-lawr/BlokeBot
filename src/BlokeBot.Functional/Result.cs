...
public sealed record Result<TValue, TError>
{
    ...
    private Result(ResultState state) => _state = state;

    public static Result<TValue, TError> Success(TValue value) => new(new SuccessState(value));
    ...
    public Result<TMapped, TError> Map<TMapped>(Func<TValue, TMapped> map) =>
        Match(value => Result<TMapped, TError>.Success(map(value)), Result<TMapped, TError>.Error);
